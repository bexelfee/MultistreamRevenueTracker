    (function () {
      ...
      function youtubeLevelsForRules() {
        const levels = (state.youtube && state.youtube.levels) || [];
        const tierPoints = (state.point_rules.youtube_membership || {}).tier_points || {};
        return levels.map((level) => ({
          key: String(level.level_id),
          name: level.name,
          label: level.name,
          points: tierPoints[level.level_id] ?? 0,
        }));
      }
      function tiersForEventType(eventKey) {
        if (eventKey.startsWith("patreon")) return patreonTiersForRules();
        if (eventKey === "youtube_membership" || eventKey === "youtube_membership_gift") {
          return youtubeLevelsForRules();
        }
        return TWITCH_TIERS;
      }
      function usesTierRuleCards(meta) {
        if (meta.key === "patreon_pledge_create") return meta.mode === "per_event_tier";
        if (meta.key === "youtube_membership") return meta.mode === "per_event_tier";
        if (meta.key === "youtube_membership_gift") return meta.mode === "per_quantity_tier";
        return false;
      }
      function emptyTierRuleHint(meta) {
        ...
        if (meta.key.startsWith("youtube_membership")) {
          return state.youtube && state.youtube.load_error
            ? state.youtube.load_error
            : "No YouTube membership levels loaded. Connect YouTube in Monitors.";
        }
      ...
      const REVENUE_TYPES = [
        ...
        { key: "youtube_membership", label: "Membership (YouTube)", mode: "per_event_tier" },
        { key: "youtube_membership_gift", label: "Membership Gift (YouTube)", mode: "per_quantity_tier" },
      ...
      const TEST_INPUTS = {
        ...
        youtube_membership: [{ key: "tier", placeholder: "Level", tierSelect: true }],
        youtube_membership_gift: [
          { key: "tier", placeholder: "Level", tierSelect: true },
          { key: "quantity", placeholder: "Gift count", int: true },
        ],
      ...
      function collectRulesFromForm() {
        ...
        for (const meta of REVENUE_TYPES) {
          ...
          if (!card) continue;
          if (meta.mode === "per_quantity") rule.points_per_unit = readNumber(card, "points_per_unit");
          else if (meta.mode === "per_eur") rule.points_per_eur = readNumber(card, "points_per_eur");
          else if (meta.mode === "per_event") rule.points_per_event = readNumber(card, "points_per_event");
          else {
            rule.tier_points = {};
